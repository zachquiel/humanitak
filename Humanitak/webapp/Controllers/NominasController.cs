        ...
        public async Task<PartialViewResult> _Autorizar(PayDayReference viewModel) {
            ...
            try {
                using (var db = new DataContext()) {
                    ...
                    viewModel.ProcessedMessage = "La nómina se autorizó correctamente";
                    return PartialView(viewModel);
                }
            }
            catch (Exception e) {
                viewModel.Success = false;
                viewModel.ProcessedMessage = "Ocurrió un error al Autorizar la nómina, intente de nuevo" + e.Message;
                return PartialView(viewModel);
            }
        }
        ...
        public async Task<PartialViewResult> _Nueva(PayDayViewModel viewModel) {
            ...
            using (var db = new DataContext()) {
                ...
                foreach (var employee in ent.Employees.ToList()) {
                    ...
                    var multiplier = 30.4166;
                    var lastPayDate = lastPayDay?.PayDate ?? (payStart.Day <= ent.Payday1End ? firstPay : secondPay);
                    var factor = 1;
